...
for Main=1400:100:1600
    i1=i1+1;
    for Period=660:20:760
        i2=i2+1;
       for PolyThick=200:20:400
           i3=i3+1;
            for d=100:10:PolyThick-10
                i4=i4+1;
                %Calculate for Pz Dipole
                Pz=1;
                model=Bullseye_Design_2D_center_flat_low(Main,Period,d,PolyThick,Pz);
                ...
                % Write data to file
                str_parameters={'Main',Main;'Period',Period;'d',d;'PolyThick',PolyThick};
                file_name=sprintf('%s%s%d%s%d%s%d%s%d%s','output/','Main_',...
                    Main,'_Period_',Period,'_d_',d,'_PolyThick_',PolyThick,'.mat');
                ...
                if  Avg_SM_coup_eff>=max_SM_Coup_eff
                    ...
                    fprintf(log_id,'Temp. Max found for Main=%d,Period=%d,d=%d,PolyThick=%d,SM coup eff for Pz=%.4f,Py=%.4f,Avg SM coup. eff=%.4f\n',...
                        Main_max,Period_max,d_max,PolyThick_max,Pz_SM_coup_eff,Py_SM_coup_eff,max_SM_Coup_eff);
                
                elseif  Avg_SM_coup_eff>=0.5
                    fprintf(log_id,'High coup. eff for Main=%d,Period=%d,d=%d,PolyThick=%d, SM coup eff for Pz=%.4f,Py=%.4f,Avg SM coup. eff=%.4f\n',...           
                        Main,Period,d,PolyThick,Pz_SM_coup_eff,Py_SM_coup_eff,Avg_SM_coup_eff);
                end
...
fprintf(log_id,'Optimization Ended,Max found for Main=%d,Period=%d,d=%d,PolyThick=%d, SM coup. eff=%f\n',...
                        Main_max,Period_max,d_max,PolyThick_max,max_SM_Coup_eff);
